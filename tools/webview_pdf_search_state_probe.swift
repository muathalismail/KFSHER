...
final class ProbeDelegate: NSObject, WKNavigationDelegate {
    ...
    private func waitReady(attempts: Int = 0) {
        let check = "document.readyState === 'complete' && typeof showExactDept === 'function'"
...
let deptKey = (args.first ?? "pediatrics")
    .replacingOccurrences(of: "\\", with: "\\\\")
    .replacingOccurrences(of: "'", with: "\\'")
let appURLString = args.dropFirst().dropFirst().first ?? "http://127.0.0.1:8000/on_call_look_up%206/"
...
(function() {
  ...
  (async function() {
    try {
      ...
      if (!preview) throw new Error('No preview button found');
      preview.click();
      await new Promise(r => setTimeout(r, 1000));
      window.__probeResult = {
        deptKey: '\(deptKey)',
        searchInputPresent: !!document.getElementById('pdfSearchInput'),
        searchButtonPresent: !!document.getElementById('pdfSearchBtn'),
        searchStatusPresent: !!document.getElementById('pdfSearchStatus'),
        searchResultsPresent: !!document.getElementById('pdfSearchResults'),
        renderStatus: document.getElementById('pdfRenderStatus')?.textContent || '',
      };
    } catch (err) {
