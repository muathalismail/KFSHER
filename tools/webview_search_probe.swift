...
final class ProbeDelegate: NSObject, WKNavigationDelegate {
    ...
    private func waitReady(attempts: Int = 0) {
        let check = "document.readyState === 'complete' && typeof search === 'function' && !!document.getElementById('cards')"
...
(function() {
  ...
  (async function() {
    try {
      ...
      const firstCard = cards ? (cards.textContent || '').trim() : '';
      const previewBtn = preview ? preview.getAttribute('data-preview') : '';
      if (preview) {
        preview.click();
        await new Promise(r => setTimeout(r, 500));
      }
      const sourceHint = document.getElementById('pdfSourceHint');
      ...
      window.__probeResult = {
        ...
        previewHint: sourceHint ? (sourceHint.textContent || '').trim() : '',
