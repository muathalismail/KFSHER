...
function parseEntPdfEntries(text='', deptKey='ent') {
  const entries = [];
  const contactResult = buildContactMapFromText(text);
  const dept = ROTAS[deptKey] || { contacts:{} };
  const { monthPad, year: detectedYr } = detectPdfMonthYear(text);

  const serverSchedule = parseEntPdfEntries._serverSchedule;
  if (Array.isArray(serverSchedule) && serverSchedule.length) {
    console.log(`[ENT] Using server-extracted schedule (${serverSchedule.length} rows)`);
    const FIELD_TO_ROLE = [
      { field: 'first_oncall',  role: '1st On-Call Resident 24h' },
      { field: 'second_oncall', role: '2nd On-Call Senior Resident 24h' },
      { field: 'third_oncall',  role: 'Consultant On-Call 24h' },
    ];
    for (const row of serverSchedule) {
      const dateKey = row.date || '';
      if (!dateKey) continue;
    ...
    const deduped = dedupeParsedEntries(entries);
    deduped._templateDetected = deduped.length >= 20;
    deduped._templateName = deduped._templateDetected ? `ent-${monthPad}-${detectedYr}` : '';
    deduped._serverExtracted = true;
    return deduped;
  }
...
function parsePalliativePdfEntries(text='', deptKey='palliative') {
  const entries = [];
  const contactResult = buildContactMapFromText(text);
  const dept = ROTAS[deptKey] || { contacts:{} };
  const { monthPad, year: detectedYr } = detectPdfMonthYear(text);

  const serverSchedule = parsePalliativePdfEntries._serverSchedule;
  if (Array.isArray(serverSchedule) && serverSchedule.length) {
    console.log(`[PALLIATIVE] Using server-extracted schedule (${serverSchedule.length} rows)`);
    const FIELD_TO_ROLE = [
      { field: 'first_oncall',  role: '1st On-Call' },
      { field: 'second_oncall', role: '2nd On-Call' },
      { field: 'consultant',    role: 'Consultant On-Call' },
    ];
    for (const row of serverSchedule) {
      const dateKey = row.date || '';
      if (!dateKey) continue;
    ...
    const deduped = dedupeParsedEntries(entries);
    deduped._templateDetected = deduped.length >= 20;
    deduped._templateName = deduped._templateDetected ? `palliative-${monthPad}-${detectedYr}` : '';
    deduped._serverExtracted = true;
    return deduped;
  }
...
function parseOphthalmologyPdfEntries(text='', deptKey='ophthalmology') {
  const entries = [];
  const contactResult = buildContactMapFromText(text);
  const dept = ROTAS[deptKey] || { contacts:{} };
  const { monthPad, year: detectedYr } = detectPdfMonthYear(text);

  const serverSchedule = parseOphthalmologyPdfEntries._serverSchedule;
  if (Array.isArray(serverSchedule) && serverSchedule.length) {
    console.log(`[OPHTHALMOLOGY] Using server-extracted schedule (${serverSchedule.length} rows)`);
    const FIELD_TO_ROLE = [
      { field: 'first_oncall', role: '1st On-Call' },
      { field: 'oncall',       role: 'On-Call' },
    ];
    for (const row of serverSchedule) {
      const dateKey = row.date || '';
      if (!dateKey) continue;
    ...
    const deduped = dedupeParsedEntries(entries);
    deduped._templateDetected = deduped.length >= 10;
    deduped._templateName = deduped._templateDetected ? `ophthalmology-${monthPad}-${detectedYr}` : '';
    deduped._serverExtracted = true;
    return deduped;
  }
