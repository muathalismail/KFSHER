...
const SPECIALTY_FILENAME_INTERPRETERS = [
  ...
  { key:'critical_care', icon:'🏥', label:'Critical Care / العناية المركزة', terms:['critical care','critical care duty','critical duty','critical may','critical','icu duty','icu rota','intensive care'] },
...
const PDF_DETECTION_RULES = [
  ...
  { key:'critical_care', terms:['critical care','critical duty','critical may','critical','icu','intensive care','icu duty','critical care duty','العناية المركزة','العناية'] },
  ...
  if (deptKey === 'liver') {
    ...
    try {
      ...
      const resp = await fetch('/api/extract-table', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pdf_base64: b64, specialty: 'gynecology' }),
      });
      if (resp.ok) {
        const result = await resp.json();
        if (result.rows && result.rows.length) {
          console.log(`[GYNECOLOGY] Server extracted ${result.rows.length} schedule rows`);
          parseGynecologyPdfEntries._serverSchedule = result.rows;
        }
      }
    } catch (err) {
